...
class VideoDatasetOptimize(Dataset):
    """Torch Video Dataset
    ...
    def __init__(self, video_file, skip_frames=None, output_size=None):
        ...
        # 整体读入所有帧，不考虑内存资源 
        self.frames = self.load_frames(self.video_frames)
    ...
    def __getitem__(self, idx):
        # Get the frame data and frame number respective skip_frames
        frame_idx, frame_data = self.frames[idx]
    ...
    def load_frames(self, video_frames):
        """Load in multiple frames from the video using a lazy generator

        Args:
            video_frames (list): list of frame numbers to load

        Returns:
            list: list of frame data tensors
        """
        container = av.open(self.file_name)
        stream = container.streams.video[0]

        frame_data_list = []
        target_frames = set(video_frames)

        for i, frame in enumerate(container.decode(stream)):
            if i in target_frames:
                frame_data = torch.from_numpy(frame.to_ndarray(format="rgb24"))
                frame_data_list.append([i, frame_data])

            # 提前结束，避免全视频解码
            if len(frame_data_list) == len(target_frames):
                break
